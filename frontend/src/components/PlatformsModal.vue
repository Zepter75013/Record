<!-- frontend/src/components/PlatformsModal.vue -->
<template>
  <Teleport to="body">
        ...
        <div class="modal-card" :class="{ 'is-saving': saving }">
          <div class="modal-header">
            <div class="header-content">
              <h2>{{ isEditing ? '✏️ Modifier la plateforme' : '➕ Ajouter une plateforme' }}</h2>
              <p class="modal-subtitle">
                {{ isEditing ? 'Modifiez les informations de la plateforme' : 'Ajoutez une nouvelle plateforme à votre base' }}
              </p>
            </div>
            <button
              @click="handleClose"
          ...
          <form @submit.prevent="handleSave" class="platform-form">
            <div class="form-section">
              <h3 class="section-title">🎮 Informations de la plateforme</h3>
              <div class="form-grid">
                <label class="form-field">
                  <span class="required">Nom de la plateforme</span>
                  <div class="input-with-suggestion">
                    <input
                      ...
                      type="text"
                      required
                      placeholder="Ex: PS5, Xbox Series X|S, Nintendo Switch..."
                ...
                <label class="form-field form-field-full">
                  ...
                  <textarea
                    ...
                    rows="3"
                    placeholder="Détails, fabricant, génération..."
                    maxlength="500"
                    :disabled="saving"
                  ></textarea>
    ...
    <div v-if="isUnsavedChangesModalOpen" class="modal-overlay" @click.self="cancelClose">
      <div class="modal-card">
        ...
        <div class="info-section">
          ...
          <p>Vous avez commencé à modifier la plateforme. Si vous fermez maintenant, ces modifications seront perdues.</p>
        </div>
        <div class="modal-actions">
      ...
      <div class="modal-card">
        <div class="modal-header">
          <div class="header-title-container">
            <span class="title-icon">⚠️</span>
            <div>
              <h2>Plateforme déjà existante</h2>
              <p class="header-subtitle">
                Une plateforme avec ce nom existe déjà dans votre collection
              </p>
            </div>
          </div>
        ...
        <div class="info-section duplicate-warning">
          <h4>🎮 Plateforme en conflit</h4>
          <p>Le nom "<strong>{{ duplicatePlatformName }}</strong>" est déjà utilisé par une plateforme existante dans votre collection.</p>
          <p class="help-text">Pour créer cette plateforme, vous devez choisir un nom différent.</p>
        </div>
        <div class="modal-actions">
...
const emit = defineEmits(['close', 'platform-saved']);
...
const duplicatePlatformName = ref('');
...
// Computed
const isEditing = computed(() => !!props.platformData?.id);
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      const response = await useApi().put(`/platforms/${props.platformData.id}`, dataToSend);
      savedPlatform = response.data || response;
    } else {
      const response = await useApi().post('/platforms', dataToSend);
      savedPlatform = response.data || response;
    }

    emit('platform-saved', savedPlatform);
    emit('close');

  } catch (error) {
    console.error('❌ [PlatformsModal] Erreur lors de la sauvegarde:', error);
    ...
    if (isDuplicate) {
      duplicatePlatformName.value = formData.value.name.trim();
...
const handleClose = () => {
  ...
  const originalName = props.platformData?.name || '';
  const originalDescription = props.platformData?.description || '';
...
const initializeForm = () => {
  if (props.platformData?.id) {
    formData.value = {
      ...props.platformData,
      created_at: props.platformData.created_at || '',
      updated_at: props.platformData.updated_at || ''
    };
  ...
  } else if (props.platformData?.name) {
    formData.value = {
      id: null,
