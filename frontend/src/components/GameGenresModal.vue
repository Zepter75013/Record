<!-- frontend/src/components/GameGenresModal.vue -->
<template>
  <Teleport to="body">
        ...
        <div class="modal-card" :class="{ 'is-saving': saving }">
          <div class="modal-header">
            <div class="header-content">
              <h2>{{ isEditing ? '✏️ Modifier le genre de jeu' : '➕ Ajouter un genre de jeu' }}</h2>
              <p class="modal-subtitle">
                {{ isEditing ? 'Modifiez les informations du genre de jeu' : 'Ajoutez un nouveau genre de jeu à votre base' }}
              </p>
            </div>
            <button
              @click="handleClose"
          ...
          <form @submit.prevent="handleSave" class="gamegenre-form">
            <div class="form-section">
              <h3 class="section-title">🕹️ Informations du genre de jeu</h3>
              <div class="form-grid">
                <label class="form-field">
                  <span class="required">Nom du genre de jeu</span>
                  <div class="input-with-suggestion">
                    <input
                      ...
                      type="text"
                      required
                      placeholder="Ex: Action, RPG, Plateforme, Course..."
                ...
                <label class="form-field form-field-full">
                  ...
                  <textarea
                    ...
                    rows="3"
                    placeholder="Caractéristiques, exemples de jeux..."
                    maxlength="500"
                    :disabled="saving"
                  ></textarea>
    ...
    <div v-if="isUnsavedChangesModalOpen" class="modal-overlay" @click.self="cancelClose">
      <div class="modal-card">
        ...
        <div class="info-section">
          ...
          <p>Vous avez commencé à modifier le genre de jeu. Si vous fermez maintenant, ces modifications seront perdues.</p>
        </div>
        <div class="modal-actions">
      ...
      <div class="modal-card">
        <div class="modal-header">
          <div class="header-title-container">
            <span class="title-icon">⚠️</span>
            <div>
              <h2>Genre de jeu déjà existant</h2>
              <p class="header-subtitle">
                Un genre de jeu avec ce nom existe déjà dans votre collection
              </p>
            </div>
          </div>
        ...
        <div class="info-section duplicate-warning">
          <h4>🎮 Genre de jeu en conflit</h4>
          <p>Le nom "<strong>{{ duplicateGameGenreName }}</strong>" est déjà utilisé par un genre de jeu existant dans votre collection.</p>
          <p class="help-text">Pour créer ce genre de jeu, vous devez choisir un nom différent.</p>
        </div>
        <div class="modal-actions">
...
const emit = defineEmits(['close', 'game-genre-saved']);
const props = defineProps({
  isOpen: Boolean,
  gameGenreData: Object,
  prefillName: String,
  apiError: String
});
...
const duplicateGameGenreName = ref('');
...
// Computed
const isEditing = computed(() => !!props.gameGenreData?.id);
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      const response = await useApi().put(`/game-genres/${props.gameGenreData.id}`, dataToSend);
      savedGameGenre = response.data || response;
    } else {
      const response = await useApi().post('/game-genres', dataToSend);
      savedGameGenre = response.data || response;
    }

    emit('game-genre-saved', savedGameGenre);
    emit('close');

  } catch (error) {
    console.error('❌ [GameGenresModal] Erreur lors de la sauvegarde:', error);
    ...
    if (isDuplicate) {
      duplicateGameGenreName.value = formData.value.name.trim();
...
const handleClose = () => {
  ...
  const originalName = props.gameGenreData?.name || '';
  const originalDescription = props.gameGenreData?.description || '';
...
const initializeForm = () => {
  if (props.gameGenreData?.id) {
    formData.value = {
      ...props.gameGenreData,
      created_at: props.gameGenreData.created_at || '',
      updated_at: props.gameGenreData.updated_at || ''
    };
  ...
  } else if (props.gameGenreData?.name) {
    formData.value = {
      id: null,
      name: props.gameGenreData.name,
      description: props.gameGenreData.description || ''
    };
  } else {
    resetForm();
  }
