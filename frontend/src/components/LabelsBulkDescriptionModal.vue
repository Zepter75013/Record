<!-- frontend/src/components/LabelsBulkDescriptionModal.vue -->
<template>
  <Teleport to="body">
        ...
        <div class="modal-card bulk-card">
          <div class="modal-header">
            <div class="header-content">
              <h2>🔄 Mettre à jour les descriptions</h2>
              <p class="modal-subtitle">
                Cochez les labels à mettre à jour, la description sera recherchée automatiquement sur Discogs.
              </p>
            </div>
            <button
              @click="handleClose"
          ...
          <div class="bulk-toolbar">
            ...
            <div class="bulk-toolbar-actions">
              ...
              <button type="button" @click="selectMissingOnly" class="ghost-btn small" :disabled="running">Sans description uniquement</button>
            </div>
          </div>
          ...
          <div class="bulk-list">
            ...
            <label v-for="label in labels" :key="label.id" class="bulk-row" :class="{ 'is-running': results[label.id]?.status === 'loading' }">
              ...
              <span v-if="label.description" class="bulk-badge">📝 déjà renseignée</span>
...
const initSelection = () => {
  ...
  for (const label of props.labels) {
    sel[label.id] = !label.description;
  }
  selected.value = sel;
  results.value = {};
  hasRun.value = false;
...
const selectMissingOnly = () => {
  for (const label of props.labels) selected.value[label.id] = !label.description;
};
