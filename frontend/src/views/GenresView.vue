...
const sortedGenres = computed(() => {
  const dataToSort = Array.isArray(genres.value) ? genres.value : [];
...
const openModal = (genre = null) => {
  apiError.value = null;
  currentGenre.value = genre ? { ...genre } : null;
...
const handleSave = (savedGenre) => {
  const existingIndex = genres.value.findIndex((g) => g.id === savedGenre.id);
  if (existingIndex !== -1) {
    genres.value[existingIndex] = savedGenre;
  } else {
    genres.value.push(savedGenre);
  }
};

const prepareDelete = (genre) => {
...
const handleRowClick = (genre) => {
  selectedRowId.value = selectedRowId.value === genre.id ? null : genre.id;
};

const handleRowDoubleClick = (genre) => {
  selectedRowId.value = genre.id;
  openModal(genre);
};
...
<template>
  <div class="settings-genres-view">
    <header class="page-header">
      <div class="header-wrapper">
        ...
        <p class="subtitle">Gérez les genres musicaux de votre collection.</p>
        <p class="user-hint">
          <span>💡</span>
    ...
    <div class="content-panel panel">
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr
              ...
              :key="genre.id"
              :class="{ 'odd-row': index % 2 === 0, 'even-row': index % 2 !== 0, 'selected-row': selectedRowId === genre.id }"
              @click="handleRowClick(genre)"
              @dblclick="handleRowDoubleClick(genre)"
            >
              <td class="id-column">{{ genre.id }}</td>
              <td class="name-column">{{ genre.name || '-' }}</td>
              <td class="actions-column">
                <div class="action-buttons-container">
                  <button @click.stop="openModal(genre)" class="icon-action-btn edit-button">
                    <span class="icon">✏️</span>
                  </button>
                  <button @click.stop="prepareDelete(genre)" class="icon-action-btn icon-action-btn-danger delete-button">
                    <span class="icon">🗑️</span>
                  </button>
                </div>
              </td>
      ...
      <div v-else class="cards-container">
        <div
          ...
          :key="genre.id"
          class="card-item"
          :class="{ 'selected-row': selectedRowId === genre.id }"
        >
          <div class="card-main" @click="handleRowClick(genre)" @dblclick="handleRowDoubleClick(genre)">
            <div class="card-content">
              <div class="card-id">
                <span class="card-label">ID:</span>
                <span class="card-value">{{ genre.id }}</span>
              </div>
              <div class="card-name">
                <span class="card-label">Nom:</span>
                <span class="card-value">{{ genre.name || '-' }}</span>
              </div>
            </div>
          </div>
          <div class="card-actions-bottom">
            <button @click.stop="openModal(genre)" class="action-button-mobile edit-button">
              <span class="icon">✏️</span>
              <span>Modifier</span>
            </button>
            <button @click.stop="prepareDelete(genre)" class="action-button-mobile delete-button">
              <span class="icon">🗑️</span>
              <span>Supprimer</span>
            </button>
          </div>
      ...
      <div v-if="!genres || genres.length === 0" class="empty-table-message-standalone">
        Aucun genre défini.
      </div>
      <div v-else class="table-summary-wrapper-standalone">
        <div class="summary-box">Nombre : {{ genres.length }}</div>
      </div>
    </div>

    <GenresModal :is-open="isModalOpen" :genre-data="currentGenre" :api-error="apiError" @close="closeModal" @genre-saved="handleSave" />
    ...
    <Teleport to="body">
      <div v-if="isConfirmModalOpen" class="modal-overlay" @click.self="isConfirmModalOpen = false">
        <div class="modal-card">
          ...
          <div v-if="genreToDelete" class="delete-info">
            <h4>Genre à supprimer : {{ genreToDelete.name }}</h4>
          </div>
...
@media (max-width: 768px) {
  .settings-genres-view { padding: 10px; }
