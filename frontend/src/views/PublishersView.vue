<script setup>
import PublishersModal from '@/components/PublishersModal.vue';
...
const sortedPublishers = computed(() => {
  const dataToSort = Array.isArray(publishers.value) ? publishers.value : [];
...
const openModal = (publisher = null) => {
  apiError.value = null;
  currentPublisher.value = publisher ? { ...publisher } : null;
...
const handleSave = (savedPublisher) => {
  const existingIndex = publishers.value.findIndex((p) => p.id === savedPublisher.id);
  if (existingIndex !== -1) {
    publishers.value[existingIndex] = savedPublisher;
  } else {
    publishers.value.push(savedPublisher);
  }
};

const prepareDelete = (publisher) => {
...
const executeDelete = async () => {
  ...
  const id = publisherToDelete.value.id;
...
const handleRowClick = (publisher) => {
  selectedRowId.value = selectedRowId.value === publisher.id ? null : publisher.id;
};

const handleRowDoubleClick = (publisher) => {
  selectedRowId.value = publisher.id;
  openModal(publisher);
};
...
<template>
  <div class="settings-publishers-view">
    <header class="page-header">
      <div class="header-wrapper">
        ...
        <p class="subtitle">Gérez les éditeurs de votre collection de jeux vidéo.</p>
        <p class="user-hint">
          <span>💡</span>
    ...
    <div class="content-panel panel">
      <div v-if="!isMobileView" class="table-responsive">
        <table class="data-table">
          ...
          <tbody>
            <tr
              v-for="(publisher, index) in sortedPublishers"
              :key="publisher.id"
              :class="{ 'odd-row': index % 2 === 0, 'even-row': index % 2 !== 0, 'selected-row': selectedRowId === publisher.id }"
              @click="handleRowClick(publisher)"
              @dblclick="handleRowDoubleClick(publisher)"
            >
              <td class="id-column">{{ publisher.id }}</td>
              <td class="name-column">{{ publisher.name || '-' }}</td>
              <td class="actions-column">
                <div class="action-buttons-container">
                  <button @click.stop="openModal(publisher)" class="icon-action-btn edit-button">
                    <span class="icon">✏️</span>
                  </button>
                  <button @click.stop="prepareDelete(publisher)" class="icon-action-btn icon-action-btn-danger delete-button">
                    <span class="icon">🗑️</span>
                  </button>
                </div>
              </td>
      ...
      <div v-else class="cards-container">
        <div
          v-for="publisher in sortedPublishers"
          :key="publisher.id"
          class="card-item"
          :class="{ 'selected-row': selectedRowId === publisher.id }"
        >
          <div class="card-main" @click="handleRowClick(publisher)" @dblclick="handleRowDoubleClick(publisher)">
            <div class="card-content">
              <div class="card-id">
                <span class="card-label">ID:</span>
                <span class="card-value">{{ publisher.id }}</span>
              </div>
              <div class="card-name">
                <span class="card-label">Nom:</span>
                <span class="card-value">{{ publisher.name || '-' }}</span>
              </div>
            </div>
          </div>
          <div class="card-actions-bottom">
            <button @click.stop="openModal(publisher)" class="action-button-mobile edit-button">
              <span class="icon">✏️</span>
              <span>Modifier</span>
            </button>
            <button @click.stop="prepareDelete(publisher)" class="action-button-mobile delete-button">
              <span class="icon">🗑️</span>
              <span>Supprimer</span>
            </button>
          </div>
      ...
      <div v-if="!publishers || publishers.length === 0" class="empty-table-message-standalone">
        Aucun éditeur défini.
      </div>
      <div v-else class="table-summary-wrapper-standalone">
        <div class="summary-box">Nombre : {{ publishers.length }}</div>
      </div>
    </div>

    <PublishersModal :is-open="isModalOpen" :publisher-data="currentPublisher" :api-error="apiError" @close="closeModal" @publisher-saved="handleSave" />
    ...
    <Teleport to="body">
      <div v-if="isConfirmModalOpen" class="modal-overlay" @click.self="isConfirmModalOpen = false">
        <div class="modal-card">
          ...
          <div v-if="publisherToDelete" class="delete-info">
            <h4>Éditeur à supprimer : {{ publisherToDelete.name }}</h4>
          </div>
...
@media (max-width: 768px) {
  .settings-publishers-view { padding: 10px; }
