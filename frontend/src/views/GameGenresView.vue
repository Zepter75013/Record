<script setup>
import GameGenresModal from '@/components/GameGenresModal.vue';
...
const gameGenreToDelete = ref(null);
const selectedRowId = ref(null);
const API_URL = '/game-genres';
const isModalOpen = ref(false);
const currentGameGenre = ref(null);
...
const fetchGameGenres = async () => {
  try {
    ...
    gameGenres.value = Array.isArray(data) ? data : [];
  } catch (error) {
...
const sortedGameGenres = computed(() => {
  const dataToSort = Array.isArray(gameGenres.value) ? gameGenres.value : [];
...
const openModal = (gameGenre = null) => {
  apiError.value = null;
  currentGameGenre.value = gameGenre ? { ...gameGenre } : null;
...
const closeModal = () => {
  ...
  currentGameGenre.value = null;
};

const handleSave = (savedGameGenre) => {
  const existingIndex = gameGenres.value.findIndex((p) => p.id === savedGameGenre.id);
  if (existingIndex !== -1) {
    gameGenres.value[existingIndex] = savedGameGenre;
  } else {
    gameGenres.value.push(savedGameGenre);
  }
};

const prepareDelete = (gameGenre) => {
  gameGenreToDelete.value = gameGenre;
  isConfirmModalOpen.value = true;
};

const executeDelete = async () => {
  if (!gameGenreToDelete.value) return;
  const id = gameGenreToDelete.value.id;
  isConfirmModalOpen.value = false;
  try {
    await apiFetch(`${API_URL}/${id}`, { method: 'DELETE' });
    gameGenres.value = gameGenres.value.filter((p) => p.id !== id);
    if (selectedRowId.value === id) selectedRowId.value = null;
    gameGenreToDelete.value = null;
  } catch (error) {
    alert(`Échec: ${error.message}`);
  }
};

const handleRowClick = (gameGenre) => {
  selectedRowId.value = selectedRowId.value === gameGenre.id ? null : gameGenre.id;
};

const handleRowDoubleClick = (gameGenre) => {
  selectedRowId.value = gameGenre.id;
  openModal(gameGenre);
};
...
<template>
  <div class="settings-gamegenres-view">
    <header class="page-header">
      <div class="header-wrapper">
        ...
        <p class="subtitle">Gérez les genres de jeux vidéo de votre collection.</p>
        <p class="user-hint">
          <span>💡</span>
    ...
    <div class="content-panel panel">
      <div v-if="!isMobileView" class="table-responsive">
        <table class="data-table">
          ...
          <tbody>
            <tr
              v-for="(gameGenre, index) in sortedGameGenres"
              :key="gameGenre.id"
              :class="{ 'odd-row': index % 2 === 0, 'even-row': index % 2 !== 0, 'selected-row': selectedRowId === gameGenre.id }"
              @click="handleRowClick(gameGenre)"
              @dblclick="handleRowDoubleClick(gameGenre)"
            >
              <td class="id-column">{{ gameGenre.id }}</td>
              <td class="name-column">{{ gameGenre.name || '-' }}</td>
              <td class="actions-column">
                <div class="action-buttons-container">
                  <button @click.stop="openModal(gameGenre)" class="icon-action-btn edit-button">
                    <span class="icon">✏️</span>
                  </button>
                  <button @click.stop="prepareDelete(gameGenre)" class="icon-action-btn icon-action-btn-danger delete-button">
                    <span class="icon">🗑️</span>
                  </button>
                </div>
              </td>
      ...
      <div v-else class="cards-container">
        <div
          v-for="gameGenre in sortedGameGenres"
          :key="gameGenre.id"
          class="card-item"
          :class="{ 'selected-row': selectedRowId === gameGenre.id }"
        >
          <div class="card-main" @click="handleRowClick(gameGenre)" @dblclick="handleRowDoubleClick(gameGenre)">
            <div class="card-content">
              <div class="card-id">
                <span class="card-label">ID:</span>
                <span class="card-value">{{ gameGenre.id }}</span>
              </div>
              <div class="card-name">
                <span class="card-label">Nom:</span>
                <span class="card-value">{{ gameGenre.name || '-' }}</span>
              </div>
            </div>
          </div>
          <div class="card-actions-bottom">
            <button @click.stop="openModal(gameGenre)" class="action-button-mobile edit-button">
              <span class="icon">✏️</span>
              <span>Modifier</span>
            </button>
            <button @click.stop="prepareDelete(gameGenre)" class="action-button-mobile delete-button">
              <span class="icon">🗑️</span>
              <span>Supprimer</span>
            </button>
          </div>
      ...
      <div v-if="!gameGenres || gameGenres.length === 0" class="empty-table-message-standalone">
        Aucun genre de jeu défini.
      </div>
      <div v-else class="table-summary-wrapper-standalone">
        <div class="summary-box">Nombre : {{ gameGenres.length }}</div>
      </div>
    </div>

    <GameGenresModal :is-open="isModalOpen" :game-genre-data="currentGameGenre" :api-error="apiError" @close="closeModal" @game-genre-saved="handleSave" />
    ...
    <Teleport to="body">
      <div v-if="isConfirmModalOpen" class="modal-overlay" @click.self="isConfirmModalOpen = false">
        <div class="modal-card">
          ...
          <div v-if="gameGenreToDelete" class="delete-info">
            <h4>Genre de jeu à supprimer : {{ gameGenreToDelete.name }}</h4>
          </div>
...
@media (max-width: 768px) {
  .settings-gamegenres-view { padding: 10px; }
