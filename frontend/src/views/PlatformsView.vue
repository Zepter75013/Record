<script setup>
import PlatformsModal from '@/components/PlatformsModal.vue';
...
const sortedPlatforms = computed(() => {
  const dataToSort = Array.isArray(platforms.value) ? platforms.value : [];
...
const openModal = (platform = null) => {
  apiError.value = null;
  currentPlatform.value = platform ? { ...platform } : null;
...
const handleSave = (savedPlatform) => {
  const existingIndex = platforms.value.findIndex((p) => p.id === savedPlatform.id);
  if (existingIndex !== -1) {
    platforms.value[existingIndex] = savedPlatform;
  } else {
    platforms.value.push(savedPlatform);
  }
};

const prepareDelete = (platform) => {
...
const executeDelete = async () => {
  ...
  const id = platformToDelete.value.id;
...
const handleRowClick = (platform) => {
  selectedRowId.value = selectedRowId.value === platform.id ? null : platform.id;
};

const handleRowDoubleClick = (platform) => {
  selectedRowId.value = platform.id;
  openModal(platform);
};
...
<template>
  <div class="settings-platforms-view">
    <header class="page-header">
      <div class="header-wrapper">
        ...
        <p class="subtitle">Gérez les plateformes de votre collection de jeux vidéo.</p>
        <p class="user-hint">
          <span>💡</span>
    ...
    <div class="content-panel panel">
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr
              ...
              :key="platform.id"
              :class="{ 'odd-row': index % 2 === 0, 'even-row': index % 2 !== 0, 'selected-row': selectedRowId === platform.id }"
              @click="handleRowClick(platform)"
              @dblclick="handleRowDoubleClick(platform)"
            >
              <td class="id-column">{{ platform.id }}</td>
              <td class="name-column">{{ platform.name || '-' }}</td>
              <td class="actions-column">
                <div class="action-buttons-container">
                  <button @click.stop="openModal(platform)" class="icon-action-btn edit-button">
                    <span class="icon">✏️</span>
                  </button>
                  <button @click.stop="prepareDelete(platform)" class="icon-action-btn icon-action-btn-danger delete-button">
                    <span class="icon">🗑️</span>
                  </button>
                </div>
              </td>
      ...
      <div v-else class="cards-container">
        <div
          ...
          :key="platform.id"
          class="card-item"
          :class="{ 'selected-row': selectedRowId === platform.id }"
        >
          <div class="card-main" @click="handleRowClick(platform)" @dblclick="handleRowDoubleClick(platform)">
            <div class="card-content">
              <div class="card-id">
                <span class="card-label">ID:</span>
                <span class="card-value">{{ platform.id }}</span>
              </div>
              <div class="card-name">
                <span class="card-label">Nom:</span>
                <span class="card-value">{{ platform.name || '-' }}</span>
              </div>
            </div>
          </div>
          <div class="card-actions-bottom">
            <button @click.stop="openModal(platform)" class="action-button-mobile edit-button">
              <span class="icon">✏️</span>
              <span>Modifier</span>
            </button>
            <button @click.stop="prepareDelete(platform)" class="action-button-mobile delete-button">
              <span class="icon">🗑️</span>
              <span>Supprimer</span>
            </button>
          </div>
      ...
      <div v-if="!platforms || platforms.length === 0" class="empty-table-message-standalone">
        Aucune plateforme définie.
      </div>
      <div v-else class="table-summary-wrapper-standalone">
        <div class="summary-box">Nombre : {{ platforms.length }}</div>
      </div>
    </div>

    <PlatformsModal :is-open="isModalOpen" :platform-data="currentPlatform" :api-error="apiError" @close="closeModal" @platform-saved="handleSave" />
    ...
    <Teleport to="body">
      <div v-if="isConfirmModalOpen" class="modal-overlay" @click.self="isConfirmModalOpen = false">
        <div class="modal-card">
          ...
          <div v-if="platformToDelete" class="delete-info">
            <h4>Plateforme à supprimer : {{ platformToDelete.name }}</h4>
          </div>
...
@media (max-width: 768px) {
  .settings-platforms-view { padding: 10px; }
