...
func (r *MySQLLabelRepository) Create(ctx context.Context, label *labels.Label) error {
	query := `INSERT INTO records_labels (name, description, country_id, founding_year, website, created_at, updated_at)
              VALUES (?, ?, ?, ?, ?, NOW(), NOW())`

	result, err := r.db.ExecContext(ctx, query, label.Name, label.Description, label.CountryID, label.FoundingYear, label.Website)
	if err != nil {
		return err
	}
...
func (r *MySQLLabelRepository) FindAll(ctx context.Context) ([]labels.Label, error) {
	query := `
		SELECT l.id, l.name, l.description, l.country_id, c.name, l.founding_year, l.website, l.created_at, l.updated_at
	...
	for rows.Next() {
		var l labels.Label
		err := rows.Scan(&l.ID, &l.Name, &l.Description, &l.CountryID, &l.CountryName, &l.FoundingYear, &l.Website, &l.CreatedAt, &l.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (r *MySQLLabelRepository) FindByID(ctx context.Context, id int) (*labels.Label, error) {
	...
	var l labels.Label
	err := row.Scan(&l.ID, &l.Name, &l.Description, &l.CountryID, &l.CountryName, &l.FoundingYear, &l.Website, &l.CreatedAt, &l.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *MySQLLabelRepository) Update(ctx context.Context, label *labels.Label) error {
	query := `UPDATE records_labels SET name = ?, description = ?, country_id = ?, founding_year = ?, website = ?, updated_at = NOW() WHERE id = ?`

	_, err := r.db.ExecContext(ctx, query, label.Name, label.Description, label.CountryID, label.FoundingYear, label.Website, label.ID)
	return err
}
